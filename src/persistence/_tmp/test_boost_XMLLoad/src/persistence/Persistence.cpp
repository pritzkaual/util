...
std::shared_ptr<ecore::EObject> Persistence::load ( const std::string& filename, std::set<std::string> options ) {
	...
	if ( true ) {
		...
		std::shared_ptr<persistence::XMLLoad> tmp_load = std::make_shared<persistence::XMLLoad>();
		//tmp_load->setOptions( options );
		//tmp_load->setFilename( filename );
		//tmp_load->setModel( model );
		//tmp_load->setMetaMetaPackage( metaMetaPackage );
